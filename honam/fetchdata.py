...
def fetch_YEAR(path='./data/', test_size=51630, fold=0):
    ...
    X_test = pd.DataFrame(X_test)
    return dict(
        X_train=X_train, y_train=y_train.astype(np.float32),
...
def fetch_SUPPORT2(path='./data/', fold=0):
    ...
    df.loc[df['income'].isna(), 'income'] = 'NaN'
    df.loc[df['income'] == 'under $11k', 'income'] = ' <$11k'
    df.loc[df['race'].isna(), 'race'] = 'NaN'
...
def fetch_fico(path='./data/', fold=0):
    ...
    if not pexists(data_path):
        ...
        file_id = "16keHIu0OSwi9v-7mlJYdsdyL-GYBetym"
        download_file_from_google_drive(file_id, data_path)

    df = pd.read_csv(data_path)
    df.loc[df["RiskPerformance"] == "Bad", 'RiskPerformance'] = 0
    df.loc[df["RiskPerformance"] == "Good", 'RiskPerformance'] = 1

    x = df.drop("RiskPerformance", axis=1)
    y = df["RiskPerformance"].values.astype(np.int32)
...
def fetch_house_prices(path='./data/', fold=0):
    ...
    cat_features = ['MSSubClass', 'MSZoning', 'Street', 'LotShape', 'LandContour', 'Utilities', 'LotConfig',
...
DATASETS = {
    ...
    "house": fetch_house_prices,
    "cahousing": fetch_ca_housing
}
